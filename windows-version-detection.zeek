...
event HTTP::log_http(rec: HTTP::Info) &priority=5
        {
        if ( rec?$host && rec?$user_agent && (/crl.microsoft.com/ in rec$host || /ocsp/ in rec$host) &&
             /Microsoft-CryptoAPI\// in rec$user_agent )
                ...
                if ( rec$user_agent !in crypto_api_mapping )
                        ...
                        Software::found(rec$id, [$version=result$version, $name=result$name, $host=rec$id$orig_h, $software_type=WINDOWS, $unparsed_version=rec$user_agent]);
                        }
                }
        }
